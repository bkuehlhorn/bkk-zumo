...
class ProximitySensors:
    ...
    def check_distance(self):
        ...
        for sensor in list(self.sensors.keys()):
            sensor_values.append([sensor, self.sensors[sensor][1]])
            current_value = self.sensors[sensor][0]()
            save_sensor_values.append([current_value, self.sensors[sensor][1]])
            if current_value > self.sensors[sensor][1]:
                proximity_triggers.append(self.sensors[sensor][2]['closer'])
                self.sensors[sensor][1] = current_value
            if current_value < self.sensors[sensor][1]:
                proximity_triggers.append(self.sensors[sensor][2]['farther'])
                self.sensors[sensor][1] = current_value
